...
# ============================================================
print("\n[4] model_hash 完整性")
sample_hash = str(supplement["model_hash"].iloc[0])
check("model_hash为64位", len(sample_hash) == 64,
      f"长度={len(sample_hash)}, 值={sample_hash[:16]}...")
...
# score_evidence 中的 hash 也检查
sample_evidence = str(supplement["score_evidence"].iloc[0])
check("score_evidence包含完整hash", supplement_hash in sample_evidence.lower(),
      "score_evidence中包含64位SHA-256")
