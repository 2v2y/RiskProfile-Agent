...
class OrchestratorGraph:
    ...
    def _load_registries(self) -> tuple[dict[str, str], dict[str, str]]:
        ...
        agent_path = self.root / reg.get("agent_registry", "configs/agent_registry.yaml")
        prompt_path = self.root / reg.get("prompt_registry", "configs/prompt_registry.yaml")
        if agent_path.exists():
            data = yaml.safe_load(agent_path.read_text(encoding="utf-8"))
            agent_versions = {a["name"]: str(a.get("version", "")) for a in data.get("agents", [])}
        if prompt_path.exists():
            data = yaml.safe_load(prompt_path.read_text(encoding="utf-8"))
            prompt_versions = {p["prompt_id"]: str(p.get("version", "")) for p in data.get("prompts", [])}
    ...
    def _versions(self) -> dict[str, Any]:
        knowledge: dict[str, Any] = {"note": "学生2阶段6交付 5.0-frozen"}
        manifest_path = self.root / "knowledge" / "knowledge_manifest.json"
    ...
    def run(
        ...
        run_dir = new_run_dir(
            self.root / self.config["paths"]["runs"],
            run_name,
            self.config,
