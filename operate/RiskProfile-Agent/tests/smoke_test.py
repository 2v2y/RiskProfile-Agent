...
def main() -> int:
    ...
    profile_agent = ProfileAgent(whitelist_path=ROOT / config["paths"]["whitelist"])
    leaked = dict(sample_v1)
    leaked["label"] = 1
    try:
        profile_agent.run(leaked)
        check("Profile Agent 拒绝白名单外字段", False, "未抛出异常")
    except ValueError as exc:
        check("Profile Agent 拒绝白名单外字段", "label" in str(exc), str(exc))
