...
def append_run_index(
    ...
    """在 runs/run_index.jsonl 追加一条运行索引，便于按 sample_id/config_hash 找到完整运行记录。"""
    runs_dir = Path(runs_dir)
    runs_dir.mkdir(parents=True, exist_ok=True)
    index_path = runs_dir / "run_index.jsonl"
    record = {"ts_utc": record.get("ts_utc", utc_now_iso()), **record}
