...
Stage9 自包含改造（stage9_full/src/retrieval/rag_retriever.py）：
- 知识库目录由调用方传入（默认 stage9_full/data/knowledge，环境变量
  STAGE9_KNOWLEDGE_DIR 可覆盖），不再依赖交付包目录结构；
- BGE 模型强制使用服务器本地路径（环境变量 BGE_MODEL_PATH 优先，否则
  /DATA/models/bge-small-en-v1.5），设置 HF_HUB_OFFLINE/TRANSFORMERS_OFFLINE=1，
  local_files_only=True，禁止联网下载；db_meta.json 中的 model_name
  （"BAAI/bge-small-en-v1.5"）只作版本记录，不作为加载路径；
- 检索算法（精确过滤 + section 匹配优先 + BGE 语义排序 + 前缀匹配）保持不变。
...
def _local_bge_path():
    """BGE 本地模型路径：BGE_MODEL_PATH 优先，否则服务器默认 /DATA/models/bge-small-en-v1.5。

    设置离线环境变量，保证本地缺失时明确报错而不是联网下载。
    """
    os.environ.setdefault("HF_HUB_OFFLINE", "1")
    os.environ.setdefault("TRANSFORMERS_OFFLINE", "1")
    return os.getenv("BGE_MODEL_PATH") or "/DATA/models/bge-small-en-v1.5"


def _load_sentence_transformer(model_path):
    """以完全离线方式加载本地 BGE 模型。"""
    ...
    try:
        ...
        # 旧版 sentence-transformers 不接受 local_files_only：
        # 靠 HF_HUB_OFFLINE/TRANSFORMERS_OFFLINE 保证不联网。
...
class RAGRetriever:
    ...
    def __init__(self, knowledge_dir=None):
        ...
        knowledge_dir：知识库根目录（须含 vector_db/ 与 chunks/）。缺省时使用
        stage9_full/data/knowledge（环境变量 STAGE9_KNOWLEDGE_DIR 可覆盖）。
        """
        if knowledge_dir is None:
            knowledge_dir = os.getenv("STAGE9_KNOWLEDGE_DIR") or os.path.join(
        ...
        # 加载BGE模型（本地路径，禁止联网下载）
        self.model = _load_sentence_transformer(_local_bge_path())
